...
- (BOOL)isValidBWPassword
{
//    NSString *regex = @"[A-Za-z0-9_^@#$]{6,14}";
//    NSString *regex2 = @"^.*[(^)|(#)|($)|(@)].*$";
//    NSString *regex3 = @".*?[A-Za-z]+.*?";
//    NSString *regex4 = @".*?[0-9]+.*?";
//
...
//    NSString *hanzi = containChinese ? @"\u4e00-\u9fa5" : @"";
    ...
    NSString *hanzi = @"";
    if (containChinese) {
        hanzi = [NSString stringWithFormat:@"\u4e00-\u9fa5%@",other];
    }
...
- (BOOL)isValidProvinceWithMinLenth:(NSInteger)minLenth
                   maxLenth:(NSInteger)maxLenth
             containChinese:(BOOL)containChinese
        firstCannotBeDigtal:(BOOL)firstCannotBeDigtal;
{
...
//    NSString *hanzi = containChinese ? @"\u4e00-\u9fa5" : @"";
    ...
    NSString *regex = [NSString stringWithFormat:@"%@[%@A-Za-z_]{%d,%d}", first, hanzi, (int)(minLenth-1), (int)(maxLenth-1)];
    return [self isValidateByRegex:regex];
}
...
/**
 *  匹配中文，英文字母和数字及空格 并限制字数
 *
 *  @return YES 成功 NO 失败
 */
- (BOOL)isChineseAndLetterAndNumberWithMinLenth:(NSInteger)minLenth
                                       maxLenth:(NSInteger)maxLenth
            containOtherCharacter:(NSString *)containOtherCharacter{
    NSString *lengthRegex = [NSString stringWithFormat:@"{%@,%@}", @(minLenth), @(maxLenth)];
    NSString *other = @"➋➌➍➎➏➐➑➒";     //九宫格的输入值
    NSString *regEx = [NSString stringWithFormat:@"[\u4e00-\u9fa5_a-zA-Z0-9%@%@]%@",other,
                       containOtherCharacter,lengthRegex];
