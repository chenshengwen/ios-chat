...
#import "BWBetwayCardValidator.h"

@implementation BWBetwayCardValidator
...
- (BOOL)validateCharacter:(NSString *)string textField:(UITextField *)textField range:(NSRange)range
{
    ...
    NSInteger maxLen = 20;
    /*   判断string中是否包含- 复制粘贴的时候   */
    string = [string stringByReplacingOccurrencesOfString:@"-" withString:@""];
    NSInteger newLen = textField.text.length + string.length - range.length;
    if (newLen > maxLen) {
        self.errorMessage = @"输入的银行卡号不能超过16位哦!";
        //        [self showErrorMessage:self.errorMessage];
        return NO;
    }
    
    // 验证输入的内容是否为 数字组成
    if ([BWValidatorTool validatorNumbers:string]) {
        // 格式化银行卡号
        return [self formatTextField:textField validateCharacter:string range:range maxLen:maxLen textFieldType:BWInputFieldBetwayCard];
    }
    return NO;
}
