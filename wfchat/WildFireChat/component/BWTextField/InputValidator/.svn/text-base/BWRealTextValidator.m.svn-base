...
#import "BWRealTextValidator.h"

@implementation BWRealTextValidator
...
- (BOOL)validateInput:(UITextField *)input {
    ...
    if (input.text.length <= 0) {
        ...
        BOOL isMatch = [input.text isValidWithMinLenth:1 maxLenth:1000 containChinese:YES firstCannotBeDigtal:NO];
        if (isMatch == NO) {
...
- (BOOL)validateCharacter:(NSString *)string textField:(UITextField *)textField range:(NSRange)range
{
    ...
    // 验证输入的内容是否为 字母数字组成
    return [string isValidWithMinLenth:1 maxLenth:1000 containChinese:YES firstCannotBeDigtal:NO];
}
@end
