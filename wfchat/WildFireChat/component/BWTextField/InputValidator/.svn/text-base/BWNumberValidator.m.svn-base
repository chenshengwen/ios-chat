...
#import "BWNumberValidator.h"

@implementation BWNumberValidator

- (BOOL)validateInput:(UITextField *)input {
    
    if (input.text.length <= 0) {
        
        self.errorMessage = @"输入的数字不能为空!";
        ...
        BOOL isMatch = [input.text isMatch:RX(@"^(0|[1-9][0-9]*)$")];
        if (isMatch == NO) {
            
            self.errorMessage = @"请输入正确的数字!";
...
- (BOOL)validateCharacter:(NSString *)string textField:(UITextField *)textField range:(NSRange)range {
    ...
    if(range.location >= maxLen) {
        ...
        self.errorMessage = @"输入的数字已达到最大限制!";
